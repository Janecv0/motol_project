...
from core import (
    ...
    DEFAULT_LINE_BREAK_BREACHES,
    DEFAULT_LINE_BREAK_DROP_G,
    DEFAULT_LINE_BREAK_DROP_PCT,
    DEFAULT_LINE_BREAK_WINDOW,
    EMA_ALPHA,
    EXPERIMENTS_DIRNAME,
    MIN_CALIBRATION_RAW_DELTA,
    MIN_MULTIPLIER_ABS,
    NUM_MARKERS,
...
from hardware import HX711Dual, WinchMotor
from experiment_common import (
    _append_line_break_history,
    _check_line_break_safety,
    _reset_line_break_safety_state,
    _status_text_for_failure,
)
from experiment1 import _move_with_tension_feedback, _run_experiment_thread, _stabilize_tension
from experiment2 import _move_open_loop_phase, _run_experiment2_thread
from vision import find_marker_centers, get_blob_detector
...
class WinchUI:
    ...
    def _default_input_settings(self):
        return {
            ...
            "exp2_forward_steps": "20",
            "exp2_return_steps": "20",
            "exp2_dwell_ms": "500",
            "exp2_reps": "5",
            "exp2_capture_pulses": "5",
            "exp2_move_chunk_pulses": "2",
            "exp2_measurement_delay_ms": "30",
            "exp2_measurement_samples": "2",
            "exp2_sample_interval_ms": "5",
    ...
    def _collect_input_settings(self):
        ...
        if hasattr(self, "entry_forward_steps_exp2"):
            settings["exp2_forward_steps"] = self.entry_forward_steps_exp2.get()
        if hasattr(self, "entry_return_steps_exp2"):
            settings["exp2_return_steps"] = self.entry_return_steps_exp2.get()
    ...
    def _apply_loaded_input_settings(self):
        ...
        try:
            ...
            if hasattr(self, "entry_forward_steps_exp2"):
                self._set_entry_value(self.entry_forward_steps_exp2, settings["exp2_forward_steps"])
            if hasattr(self, "entry_return_steps_exp2"):
                self._set_entry_value(self.entry_return_steps_exp2, settings["exp2_return_steps"])
            if hasattr(self, "entry_dwell_exp2"):
    ...
    def _bind_input_autosave(self):
        entry_widgets = [
            ...
            self.entry_forward_steps_exp2,
            self.entry_return_steps_exp2,
    ...
    def _calibrate_scale_from_known_weight(self, scale_id):
        ...
        try:
            ...
            if not is_valid_multiplier(new_multiplier):
    ...
    def _apply_manual_multiplier(self, scale_id):
        ...
        if not is_valid_multiplier(value):
    ...
    def _read_weights_for_control(self):
        with self.scale_lock:
            if self.channel_a != self.channel_b:
                raw_a, raw_b = self.scale.read_raw_pair(self.channel_a, self.channel_b)
                w_a = (raw_a - self.tare_a) / self.cal_a
                w_b = (raw_b - self.tare_b) / self.cal_b
            else:
    ...
    def build_experiment2_tab(self):
        ...
        ttk.Label(setup_frame, text="Steps A wind / B release:").grid(row=0, column=0, sticky="w", pady=5, padx=10)
        self.entry_forward_steps_exp2 = ttk.Entry(setup_frame, width=10)
        self.entry_forward_steps_exp2.grid(row=0, column=1, sticky="w", padx=5)
        self.entry_forward_steps_exp2.insert(0, "20")
        ...
        ttk.Label(setup_frame, text="Steps A release / B wind:").grid(row=3, column=0, sticky="w", pady=5, padx=10)
        self.entry_return_steps_exp2 = ttk.Entry(setup_frame, width=10)
        self.entry_return_steps_exp2.grid(row=3, column=1, sticky="w", padx=5)
        self.entry_return_steps_exp2.insert(0, "20")
        ...
        self.entry_measurement_delay_exp2.insert(0, "30")
        ...
        self.entry_measurement_samples_exp2.insert(0, "2")
        ...
        self.entry_sample_interval_exp2.insert(0, "5")
    ...
    def _calculate_progress_pct(self, rep, phase, phase_pulses=0, phase_total=1):
        return calculate_progress_pct(
            rep,
            phase,
            phase_pulses=phase_pulses,
            phase_total=phase_total,
            reps_total=self.run_params.get("reps", 1),
        )
    ...
    def _collect_settled_samples(self, max_duration_s=None, min_samples_required=None):
        params = self.run_params
        delay_s = params["measurement_delay_ms"] / 1000.0
        interval_s = params["sample_interval_ms"] / 1000.0
        num_samples = max(1, int(params["measurement_samples"]))
        min_samples = num_samples if min_samples_required is None else max(1, int(min_samples_required))
        min_samples = min(min_samples, num_samples)
        if max_duration_s is not None:
            # In time-budgeted mode (Experiment 2), keep requested averaging but
            # cap settling delay so loop cadence stays below target.
            delay_s = min(delay_s, max_duration_s * 0.25)
        sample_start = time.perf_counter()

        def _remaining_budget():
            if max_duration_s is None:
                return None
            return max_duration_s - (time.perf_counter() - sample_start)
        ...
        if delay_s > 0:
            if max_duration_s is None:
                ...
                if remaining is not None and remaining <= 0:
                    ...
                    return None
                time.sleep(min(delay_s, max(remaining or 0.0, 0.0)))
        ...
        while len(samples_a) < num_samples and attempts < max_attempts and self.experiment_running:
            if max_duration_s is not None and len(samples_a) >= min_samples:
                remaining = _remaining_budget()
                if remaining is not None and remaining <= 0:
                    break
        ...
        if max_duration_s is None:
            ...
            measurement_valid = len(samples_a) >= min_samples
    ...
    def _get_experiment2_params(self):
        return {
            "forward_steps": int(self.entry_forward_steps_exp2.get()),
            "return_steps": int(self.entry_return_steps_exp2.get()),
    ...
    def _validate_experiment_params(self, params):
        return validate_experiment_params(params)

    def _validate_experiment2_params(self, params):
        return validate_experiment2_params(params)
    ...
    def run_experiment2(self):
        ...
        self._set_ui_state(
            ...
            phase="Init",
            rep_text=f"0 / {params['reps']}",
            pulse_text=f"0 / {params['forward_steps']}",
            correction_text="N/A",
            capture_text=capture_state,
        )
    ...
    def _stabilize_tension(self, rep):
        return _stabilize_tension(self, rep)

    def _move_with_tension_feedback(self, rep, phase):
        return _move_with_tension_feedback(self, rep, phase)

    def _run_experiment_thread(self):
        return _run_experiment_thread(self)

    def _move_open_loop_phase(self, rep, segment_index, phase, phase_total):
        return _move_open_loop_phase(self, rep, segment_index, phase, phase_total)

    def _run_experiment2_thread(self):
        return _run_experiment2_thread(self)
